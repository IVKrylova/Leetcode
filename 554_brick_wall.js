...
// each of the same height (i.e., one unit) but they can be of different widths. The total width of each row is
// the same.
// Draw a vertical line from the top to the bottom and cross the least bricks. If your line goes through the edge
// of a brick, then the brick is not considered as crossed. You cannot draw a line just along one of the two
...
const leastBricks = function (wall) {
  ...
  for (const row in board) {
    if (board[row] > maxBoard) maxBoard = board[row];
  }
  return wall.length - maxBoard;
};
