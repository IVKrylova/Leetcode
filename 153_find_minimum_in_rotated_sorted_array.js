...
const findMin = function (nums) {
  if (nums.length < 4) return Math.min(...nums);
  let start = 0;
  ...
  while (start < end) {
    ...
    if (nums[start] < nums[mid] && nums[mid + 1] < nums[end]) {
      return Math.min(nums[start], nums[mid + 1], min);
    } else if (end - start === 1) {
      return Math.min(nums[start], nums[end], min);
    } else if (end - start === 2) {
      return Math.min(nums[start], nums[end], min, nums[mid]);
