...
class RandomizedSet {
  ...
  remove(val) {
    if (this.hash[val] !== undefined) {
      if (this.array.length > 1) {
        const deletedInd = this.hash[val];
        this.hash[this.array[this.lastInd - 1]] = deletedInd;
        this.array[deletedInd] = this.array[this.lastInd - 1];
      }
      this.lastInd -= 1;
      this.array.pop();
  ...
  getRandom() {
    const ind = Math.floor(Math.random() * this.lastInd);
