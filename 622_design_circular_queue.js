...
class MyCircularQueue {
  ...
  Rear() {
    if (this.isEmpty()) return -1;
    return this.queue[this.tail === 0 ? this.maxSize - 1 : this.tail - 1];
  }
  ...
  enQueue(val) {
    ...
    this.size += 1;
    this.tail = this.tail === this.maxSize - 1 ? 0 : this.tail + 1;
    return true;
  }
  ...
  deQueue() {
    ...
    this.head = this.head === this.maxSize - 1 ? 0 : this.head + 1;
    this.size -= 1;
    return true;
  }
}
